...
testmode=False
write_output=True
today = '81519_fine_fiducial'

if len(sys.argv) < 2:
    print('syntax blah.py n_chr')
    sys.exit()

chr_no = int(sys.argv[1])
chr_list = [str(i) for i in range(1,23)] +['X', 'Y']
chrom = chr_list[chr_no]

line_no = 5 #reference to old way of calling this set of parameters
thef1 = 0
thef2 = 0
thef3 = 2
thef4 = 2
...
for i in range(geneDF[geneDFcut].shape[0]):
    ...
    if the_chr!=prev_chr:
        ...
        prev_chr = the_chr
    likelihoods, best_iteration  = search_best_likelihood(geneDF, gene, noplot=True, DHS_X=DHS_X, gene_Mixture=gene_Mixture, gene_max_x=gene_max_x, compute_on_fly=False, f1=thef1, f2=thef2, f3=thef3,f4=thef4,f5=thef5,costhresh=thecosthresh,search_space=my_search_space)
...
f = open(today+'_sweep_results_total_bychrom.txt', 'a')
print(line_no,thef1, thef2, thef3, thef4, thef5, thecosthresh, uniqueness_ratio, assignment_ratio, 'chr'+chrom, file=f)
f.close()
